...
async def create_lead_endpoint(data: LeadCreate):
    return await leads_service.create_lead(data)

@lead_router.get("/get/{lead_id}", response_model=Lead)
async def get_lead_endpoint(lead_id: int):
    return await leads_service.get_lead_by_id(lead_id)

@lead_router.get("/list", response_model=list[Lead])
async def list_leads(status: LeadStatus | None = None):
    return await leads_service.get_leads(status=status)

@lead_router.patch("/update/{lead_id}/status", response_model=Lead)
async def update_lead_status(lead_id: int, update: LeadStatusUpdate):
    return await leads_service.update_lead_status(lead_id, update)

@lead_router.delete("/delete/{lead_id}", status_code=204)
async def delete_lead(lead_id: int):
    await leads_service.delete_lead(lead_id)